console.log('QSTSS Dashboard frontend v5 Cognia Benchmarking loaded');
const state = {
  selectedQid: null,
  students: [],
  options: { qids: [], names: [] },
...
async function loadEvidence() {
  ...
  const title = evidence.selected_cohort || (cohort || 'All cohorts');
  document.getElementById('evidenceSubtitle').textContent = `Showing: ${safe(title)} · Cohort Growth, Internal Achievement, and Evidence Narrative.`;
  ...
  content.className = 'cognia-benchmark';
  content.innerHTML = `
    <section class="benchmark-hero surface">
      <div>
        <span class="version-pill">v5 Benchmarking View</span>
        <h2>QSTSS Student Progress Dashboard | ${safe(title)}</h2>
        <p class="small">Cohort Growth, Internal Achievement, and Evidence Narrative. This page is designed to match the benchmarking logic in the original Excel <strong>Cognia_Evidence_Page</strong>.</p>
      </div>
      <div class="methodology-box">
        <h3>Methodology</h3>
        <ul>${(evidence.methodology || []).map(m => `<li>${safe(m)}</li>`).join('')}</ul>
      </div>
    </section>

    <section class="evidence-summary compact benchmarking-kpis">
    ...
    <section class="benchmark-grid">
      <article class="surface evidence-table-card">
        <h2>Year profile (${safe(title)})</h2>
        ${tableFromRows(evidence.year_profile || [], [
          { key: 'Year_Index', label: 'Year Index', bold: true, format: v => formatNumber(v, 0) },
          { key: 'Avg_RAW', label: 'Avg RAW', format: v => formatNumber(v) },
          { key: 'Avg_NSIS', label: 'Avg NSIS', format: v => formatNumber(v) },
          { key: 'Avg_STEM', label: 'Avg STEM', format: v => formatNumber(v) },
          { key: 'Avg_English', label: 'Avg English', format: v => formatNumber(v) },
    ...
    <section class="evidence-table-grid">
      <article class="surface evidence-table-card">
        <h2>Entry vs exit average</h2>
        ${tableFromRows(evidence.entry_exit || [], [
          { key: 'Subject', label: 'Subject', bold: true },
    ...
    <section class="benchmark-grid">
      ...
      <article class="surface evidence-table-card insight-stack">
        <h2>Destination evidence for ${safe(title)}</h2>
        ${renderBarList('Top Universities', evidence.top_universities || [], 'University')}
        ${renderBarList('Top Majors / Specialties', evidence.top_majors || [], 'Major')}
      </article>
    </section>

    <section class="benchmark-grid">
      ${evidenceAchievementBar(evidence.schoolwide_achievements || {})}
      <article class="surface evidence-note">
        <h3>Evidence narrative link</h3>
        <p class="small">The selected cohort evidence links achievement movement, university-readiness data, CAT4/cognitive indicators, and enrichment participation. Student-level projects and award information is also shown in the Student Profile when reliable name matching is available.</p>
      </article>
    </section>
